<template>
    <div>
        <el-drawer v-model="dialogVisible" title="编辑" size="80%">
            <div class="shang">
                <el-form ref="ruleFormRef" autocomplete="off" :model="formData" :rules="rules" label-width="130px">
                    ...
                    <el-form-item label="标题图片" prop="banner">
                        ...
                        >
                            <img v-if="imageUrl" :src="imageUrl" style="max-width: 180px" class="avatar" />
                    ...
                    <el-form-item label="原价" key="78" prop="old_price" v-if="checked2">
                        <el-input type="number" v-model="formData.old_price" placeholder="请输入原价" />
                    </el-form-item>
                    <el-form-item label="折扣价" key="879" prop="price_cost" v-if="checked2">
...
<script lang="ts" setup>
    ...
    //     () => {
    //         if (formData.value?.qujian2 < formData.value?.qujian1) {
    ...
    function processLabelName(newQuery) {
        if (!newQuery || typeof newQuery.label_name !== 'string') {
            return []
        }

        const labelName = newQuery.label_name.trim()
        if (labelName.includes(',')) {
            return labelName.split(',').map((item) => item.trim())
        } else {
            return [labelName]
        }
    }
    ...
    watch(
        ...
        (newQuery, oldQuery) => {
            ...
            imageUrl.value = ''
            checkList.value = []
            const processedLabelName = processLabelName(newQuery)
            formData.value = { ...newQuery }
            imageUrl.value = 'https://api.ustudygroup.com/' + formData.value.banner
            let items = selects.value.find((item) => item.id == formData.value.type_id)
            let name = dianPuList.value.find((item) => item.shop_name == formData.value.name)
            console.log(name)
            formData.value.shop_name = name?.shop_name
            formData.value.type_name = items?.name
            checkList.value = processedLabelName
            if (formData.value.price_range) {
                if (formData.value.price_range && formData.value.price_range !== '') {
                    let arr = formData.value.price_range.split(',')
                    // 如果需要转换成数字数组，可以使用map方法
                    let numArr = arr.map(Number)
                    console.log(numArr) // 输出 [123, 423] 或 [123]
                    formData.value.qujian1 = numArr[0]
                    if (numArr[1] == 0) {
                        formData.value.qujian2 = ''
                    } else {
                        formData.value.qujian2 = numArr[1]
                    }
                }
            }
            ...
            if (formData.value.is_main == 0) {
                checked4.value = true
                checked5.value = false
            } else if (formData.value.is_main == 1) {
                checked5.value = true
                checked4.value = false
            }
