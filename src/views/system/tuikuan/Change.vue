...
<script lang="ts" setup>
    import { changeTuiList, changeShopList } from '@/api/tui/apis'
        ...
        await ruleFormRef.value.validate(async (valid, fields) => {
            if (valid) {
                try {
                    let res
                    if (role == 1) {
                        res = await changeTuiList(formData.value)
                    } else {
                        res = await changeShopList(formData.value)
                    }
                    if (res.code == 1) {
                        ElNotification({
                            message: res.msg,
